...
const fetchAll = async ({ collection, query = {}, projection = null, options = {} }) => {
  try {
...
const create = async ({ collection, body, session }) => {
  try {
    ...
    const errorMessage = err.errorResponse && err.errorResponse.errmsg && err.message;
    if (errorMessage?.includes("name_1 dup key")) {
      return [null, "Name already exists"];
    }
    logger.error(`Error while creating ${collection}: `, err);
    return [null, errorMessage || err.message || "An unexpected error occurred"];
  }
};
...
const bulkWrite = async ({ collection, operations }) => {
  try {
    ...
    logger.error(`Error while performing bulk write in ${collection}: `, err.message);
    return [null, err.message || err.errmsg];
  }
};
