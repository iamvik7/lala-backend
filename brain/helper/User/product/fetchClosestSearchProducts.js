...
const extractKeywords = async (search) => {
	const [keywordsFromDB, err] = await Db.fetchAll({
		collection: COLLECTION_NAMES.PRODUCTSEARCHKEYWORDMODEL,
		query: {},
		projection: { keyword: 1 },
	});
	if (err) {
		throw new Error(
			`Error fetching keywords from the database: ${err}`
		);
	}
	...
	const matchedKeywords = predefinedKeywords.filter(
		(keyword) => {
...
exports.fetchClosestSearchProduct = async (
	search,
	limit = 20
) => {
	try {
		...
		if (extractedKeywords.length > 0) {
			...
			let [keywordMatchResults, errForKeywordMatch] =
				await Db.aggregate({
			...
			if (errForKeywordMatch)
				return [null, errForKeywordMatch];

			if (keywordMatchResults.length > 0) {
				console.log(
					'Keyword Match Results: ',
					keywordMatchResults.length
				);
		...
		const letterFrequencyStage = searchLetters.map(
			(letter) => ({
				$addFields: {
					[`letterCount_${letter}`]: {
						$add: [
		...
		const totalFrequencyStage = {
			$addFields: {
				totalLetterFrequency: {
					$sum: searchLetters.map(
						(letter) => `$letterCount_${letter}`
					),
				},
			},
		};

		let [fallbackResults, errForFallback] =
			await Db.aggregate({
				collection: COLLECTION_NAMES.PRODUCTMODEL,
				query: [
					{ $match: { isActive: true } },
